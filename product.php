							...
							<div class="form-body">
                               ...
                               <div class="form-group">
								...
								<div class="form-group">
											...
											<select id="gurl" name="seller_id" class="form-control" required>
												<option value="" selected="" disabled="">Select Seller</option>
												<?php 
												$sl = mysqli_query($con,"select * from seller");
												while($s = mysqli_fetch_assoc($sl))
												{
												?>
												<option value="<?php echo $s['id'];?>" <?php if($s['id'] == $selk['seller_id']) { echo 'selected="selected"'; } ?>><?php echo $s['sname'];?></option>
												<?php } ?>
...
else 
{
	...
	$arr[] = "product/".$file_name;
        ...
        $con->query("update product set pname='".$pname."',seller_id='".$seller_id."',pimg='".$pimg."',prel='".$related."',popular=".$popular.",discount=".$discount.",cid=".$catname.",sid=".$subcatname.",psdesc='".$psdesc."',pgms='".$pgms."',pprice='".$pprice."',status=".$status.",stock=".$ostock." where id=".$_GET['edit']."");
									...
									<input type="text" id="gurl" class="form-control"  placeholder="Enter Seller Name" name="sname" required>
										...
										<div class="form-group">
											...
											<select id="gurl" name="seller_id" class="form-control" required>
												...
												{
												?>
												<option value="<?php echo $s['id'];?>"><?php echo $s['sname'];?></option>
												<?php } ?>
...
else 
{
	...
	$arr[] = "product/".$file_name;
		...
		$con->query("insert into product(`pname`,`pimg`,`prel`,`seller_id`,`cid`,`sid`,`psdesc`,`pgms`,`pprice`,`date`,`status`,`stock`,`discount`,`popular`)values('".$pname."','".$url."','".$related."','".$seller_id."',".$catname.",".$subcatname.",'".$psdesc."','".$pgms."','".$pprice."','".$timestamp."',".$status.",".$ostock.",".$discount.",".$popular.")");
